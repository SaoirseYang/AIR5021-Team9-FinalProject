...
spark_navigation_2d(){
    ...
    # echo -e "${Info}" 
    # echo -e "${Info}让SPARK抓取物体并前进3米" 
    # PROJECTPATH=$(cd `dirname $0`; pwd)
    # source ${PROJECTPATH}/devel/setup.bash
    # echo -e "${Info}" 
    # echo -e "${Info}请注意："
    # echo -e "${Info}       A.激光雷达已上电连接"
    # echo -e "${Info}       B.摄像头已正确连接"
    # echo -e "${Info}       C.机械臂已上电并连接" 
    # echo -e "${Info}退出请输入：Ctrl + c " 
    # echo -e "${Info}" 
    # echo && stty erase '^H' && read -p "按回车键（Enter）开始：" 
    # print_command "roslaunch spark_carry_nav carry_and_move.launch camera_type_tel:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}"
    # roslaunch spark_carry_nav carry_and_move.launch camera_type_tel:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
