...
class GraspObject(State):
    ...
    def __init__(self):
        ...
        # 添加通知抓取完成的发布者
        self.grasp_completed_pub = rospy.Publisher('/spark_carry/grasp_completed', Bool, queue_size=1)
    ...
    def grasp(self):
        ...
        pos.z = 55  # Changed from previous Z value to 150
        self.pub1.publish(pos)
        r2.sleep()
        ...
        # 发布抓取完成的消息
        grasp_completed_msg = Bool()
        grasp_completed_msg.data = True
        self.grasp_completed_pub.publish(grasp_completed_msg)
        rospy.loginfo("物体抓取完成，已发布通知")
    ...
    def image_cb(self, data):
        ...
        # # extract red (需要两个范围，因为红色在HSV色轮上横跨边界)
        # LowerRed1 = np.array([0, 100, 80])
        # UpperRed1 = np.array([10, 255, 255])
        # mask1 = cv2.inRange(cv_image2, LowerRed1, UpperRed1)

        # LowerRed2 = np.array([160, 100, 80])
        # UpperRed2 = np.array([179, 255, 255])
        # mask2 = cv2.inRange(cv_image2, LowerRed2, UpperRed2)
...
class ReleaseObject(State):
    ...
    def __init__(self):
        ...
        pub2 = rospy.Publisher('pump_topic', status, queue_size=4)   
         # 添加订阅者来接收释放物体的指令
        self.release_sub = rospy.Subscriber('/spark_carry/release_object', Bool, self.release_callback)
    ...
    def execute(self, userdata):        
        ...
        while not self.release_requested and not rospy.is_shutdown():
            rate.sleep()
